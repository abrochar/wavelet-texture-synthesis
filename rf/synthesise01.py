# synthesis color texture, observation pixel value is between [0,1]
...
from utils import get_color_mat, prep_image01
from utils_theano import gram_matrix, style_loss, style_loss_relative
from utils_theano import build_model_multiscale, build_model_one_scale

def optimize(texture_name, IMAGE_W, net, n_iter, scales):

    texture0 = get_color_mat(texture_name,0,IMAGE_W)
    pi01 = prep_image01()
    texture = pi01.forward(texture0)
    texture = floatX(texture[np.newaxis]) # (1,3,size,size)
    ...
    x0 = generated_image.get_value().astype('float64')
    synthesised = x0[0,:,:,:]
    synthesised = pi01.backward(synthesised) # (3,H,W)
...
def main():
    ...
    parser.add_argument('-n', '--n-iter', default=2000, type=int, help='Number of L-BFGS optinisation iterations')
    ...
    if not args.linear:
        net = build_model_multiscale(IMAGE_W, n_feature_maps, scales, nobias=True) # default for nobias=False
    else:
        net = build_model_multiscale(IMAGE_W, n_feature_maps, scales, nonlinearity=None)

    synthesised = optimize(texture_name, IMAGE_W, net, n_iter, scales)

    if args.target_file is None:
        target_file = texture_name.split('.')[0] +\
                      '_rf_default_nit' + str(n_iter) +\
                      '_size' + str(IMAGE_W) + '_nohist.mat'
    else:
