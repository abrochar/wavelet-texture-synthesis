...
def get_color_corr(im):

    M = im.shape[-1]; N = im.shape[-2]
    x = im.view(3,M,N)
    mean_obs = x.mean(dim=(-2,-1), keepdim=True)
    std_obs = x.std(dim=(-2,-1), keepdim=True)
    x = x - x.mean(dim=(-2,-1), keepdim=True)
    x = x / x.std(dim=(-2,-1), keepdim=True)
    U = x.view(3, M*N)
    Ut = U.transpose(0,1)  # (M*N,3)
    C = torch.matmul(U, Ut)/M/N
...
def color_corr_loss(C, x0):

    C0 = get_color_corr(x0)
    loss = (C - C0)**2
    loss = loss.sum() #*1e3
    loss.backward()
...
def call_lbfgs2_routine(x0,j,wph_ops,wph_streams,Sims,C,nb_restarts,maxite,factr, \
